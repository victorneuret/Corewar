...
bool asm_arg_direct(token_t *token, uint8_t *asm_token, int k, int *i)
{
	...
	int sum = bytes_sum(asm_token[*i], asm_token[*i + 1],
				asm_token[*i + 2], asm_token[*i + 3]);
	...
	switch (k + 1) {
	case 1: token->arg_one = sum; break;
	case 2: token->arg_two = sum; break;
	case 3: token->arg_three = sum; break;
	}
